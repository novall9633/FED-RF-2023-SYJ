...
function SubTitle(props) { // tit - 서브타이틀 / menu - 서브 메뉴

    // 서브메뉴 있을 경우 li 데이터 생성하기
    // 전달변수 data에 들어오는 값은 메뉴 배열임
    // 배열.map(v=>코드) -> html 코드 생성후 리턴됨
    const makeList = (data) => 
        data.map(v=>
                <li>
                    <a href="#">{v}</a>
...
function ItemList(props) {
    ...
    return (
        //  2-1. 카테고리 페이지 콘텐츠영역
        // html출력일 경우 dangerouslySetInnerHTML을 사용한다
        <div className={"cat-cont-area "+props.cname }>
            <section className="pt2">
                ...
                <div className="cbx bgi bg1-2">
                    {<h2>dangerouslySetInnerHTML={{ __html:props.tit[1]}}</h2>}
                </div>
                <div className="cbx bgi bg1-3">
                    {<h2>dangerouslySetInnerHTML={{ __html:props.tit[2]}}</h2>}
                </div>
            </section>
            <section className="pt2">
                <div className="cbx bgi bg2-1">
                    {<h2>dangerouslySetInnerHTML={{ __html:props.tit[3]}}</h2>}
                </div>
                <div className="cbx bgi bg2-2">
                    {<h2>dangerouslySetInnerHTML={{ __html:props.tit[4]}}</h2>}
                </div>
                <div className="cbx bgi bg2-3">
                    {<h2>dangerouslySetInnerHTML={{ __html:props.tit[5]}}</h2>}
                </div>
            </section>
        </div>
    );
