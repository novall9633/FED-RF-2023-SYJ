...
function MainComponent(){
    ...
    // 3. 선택아이템 고유 번호 상태관리변수
    const [selItem,setSelItem] = React.useState(0);
    // 4. JS 효과적용여부 상태관리 변수
    const [effctOK,setEffectOK] = React.useState(1);
    ////////////////////////////////////////////////////////////
    ...
    React.useLayoutEffect(()=>{if(effctOK)initFn()});

    // 처음 한번만 타이틀 글자 커졌다가 작아지기
    React.useEffect(()=>{if(effctOK)firstOneFn()},[]);
    ...
    /*****************************************************************
        ...
        기능 : 상태관리 변수 중 데이터 선택번호
        업데이트를 하여 데이터를 화면에 상품리스트를 업데이트 되도록 한다
    *****************************************************************/
    ...
    /*****************************************************************
        ...
        기능 : 상태관리 변수 중 리스트/ 상세보기 선택변수를
        업데이트를 하여 실제 뷰를 전환한다
    *****************************************************************/
    const chgSubView = (num,idx) => {
        // a요소 기본이동막기
        event.preventDefault();
        console.log('바꿔!',num,'/n고유번호: ',idx);
        // 1. 리스트/상세보기 뷰 상태관리 변수 변경하기
        setSubView(num);
        // 2. 선택아이템고유번호 변경
        setSelItem(idx);
        // 3. JS 효과상태변경
...
// 서브 컴포넌트(자식컴포넌트-> 
// 부모 컴포넌트로 부터 데이터를 props 속성을 통하여 전달한다.)
...
function SubViewCode(props){
    // props.idx - 선택데이터 순번(신발/드레쓰)
    // props.chgFn() 함수로 사용가능 -> 부모 chgSubView() 함수로 호출하는 것임
    // -> 프롭스 다운, 프롭스 펑션 업/다운
    ...
    // -> 고유 데이터인 idx 값으로 데이터를 찾아서 데이터를 화면에 바인딩해야한다.
    // 코드 리턴 파트
    return(
            <ol style={{display:'flex',listStyle:'none'}}>
                <li>
                    <img src={props.idx?"./images/gallery/"+selItem.idx+".jpg":"./images/vans/vans_"+selItem.idx+".jpg"} 
                    alt={props.idx?"드레스":"신발"} />
                </li>
                <li style={{lineHeight:'8',padding:'10px'}}>
                    상품명 : {selItem.gname} <br />
                    가격 : {selItem.gprice.toLocaleString('ko-KR')}원 <br/>
