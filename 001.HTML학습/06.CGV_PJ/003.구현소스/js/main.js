...
console.log("대상:",pmenu,"/스크린:",screen);
...
pmenu.forEach((ele)=>{
    domFn.addEvt(ele,'click',(e)=>{
        ...
        // 클릭된 a요소 하위 영화제목정보 읽어오기
        let mtit = domFn.qsEl(ele,'li').innerText;

        // 2. 스크린 박스에 아이프레임 src 재구성하여 넣기
        screen.innerHTML = `<iframe src="https://www.youtube.com/embed/${mvCode[mtit]}?autoplay=1&mute=1" allow="autoplay"></iframe>`;

        // 3.추가구현 : 클릭된 a요소의 부모인 li에 클래스 on 주기
        // - on을 주면 미리셋팅된 대로 li가 일어나 있다.
