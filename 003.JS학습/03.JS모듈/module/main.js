...
// -> from 뒤에 파일명 확장자 사용해야함
// -> SPA 모듈 개발환경에서는 .js, .jsx 등 확장자 생략 가능

// 텍스트 데이터
// import { mTitle,sTitle,personInfo,mvData,per} from "./text_data.js";

// 별칭으로 받기 {원래 변수 as 별칭}
import { mTitle as mTit,sTitle as sTit,personInfo as pInfo ,mvData as mv,per} from "./text_data.js";

// 메시지 구성함수
import {message as msgFn,mess} from "./msg_format.js";
// import {message as msgFn} from "./msg_format.js";
// default로 내보낸 것은 {변수} 형식으로 받을 수 없다.

console.log(mTit,sTit,pInfo,mv,per,msgFn, mess);
// 별칭으로 받기 {원래 변수 as 별칭}
...
<h3>${sTit}</h3>
`;
per.forEach(val=>titBox.innerHTML += `<h1>${mess(val[0],val[1],val[2])}`)

// (2) 내용 넣기 : 이름과 나이를 소개하는 메시지 넣기
contBox.innerHTML = msgFn('공유',45);
contBox.innerHTML += msgFn('톰행크스',59);
contBox.innerHTML += msgFn('안젤리나졸리',38);
// 이름과 나이가 셋팅된 배열 데이터를 이용하여 출력하기
pInfo.forEach((val)=>{
    contBox.innerHTML+=msgFn(val[0],val[1]);
});
