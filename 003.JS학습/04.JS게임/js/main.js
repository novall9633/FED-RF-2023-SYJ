...
// console.log(t1,r1,btns,level,msg);
...
function goR1(){
    // autoI변수에 할당전 상태는 undefined 이므로 false 처리됨-> 이때 if문으로 들어가려면 !(NOT)연산자 사용
   if(!autoI){ // false일 때 들어감
    // console.log(autoI);
    // 인터벌 실행하기
    autoI = setInterval(()=>{
...
/***************************************** 
    ...
    기능: 기준값 보다 레이서위치값이 큰경우
        승자를 판별하여 메시지를 보여준다!
*****************************************/
let t1Stop = 0; //거북 멈춤 값(1-멈춤,0이면 허용)
...
function whoWinner(){
    ...
    if(r1pos>= 650 || t1pos >= 650){
        ...
        console.log("랜덤 숫자:",rdmNum,"개수:",selMsg.length);
        // selMsg -> 선택된 메시지 배열
