...
function showTime() {
    ...
    // 찍어보면 기존 달보다 1작다 이것은 배열순번에 넣고 찍기 좋도록 월 순번이 리턴된다.
    tt[1].innerText = today.getMonth() + 1;
    // 따라서, 숫자월을 찍고 싶으면 +1을 한다.
    ...
    // 12보다 크면 12를 빼주고 작으면 한자리일때 12를 빼준다
...
// 1부터 45사이 난수 발생하기
// 방법 : 난수에 발생할 최대수 곱하기 -> 올림 or 내림
rdm = rdm*45;
console.log("난수*45",rdm);
console.log("난수*45 내림",Math.floor(rdm));
console.log("난수*45 내림",Math.ceil(rdm));
// ->1부터 최대수는 올림처리/ 0부터 최대수-1은 내림처리

for(let i=0;i<7;i++){
    lotto.push(Math.ceil(Math.random()*45));
}
console.log(lotto.sort().join(','));
...
// 2초 간격으로 인터벌 호출
setInterval(colorImg,400);

// 직전 랜덤수 : 초기셋팅은 0~3사이수가 아닌 수
let bNum = 999;

function colorImg(){

    // 1. 난수 만들기 0~3 -> 1~4를 만들고 내림처리
    let rdmNum = Math.floor(Math.random()*4);
    console.log("랜덤수:",rdmNum);
    
    // 2. 직전 랜덤수 같을 경우 다시 난수 발생하기
    // while(조건true){코드} 사용하기
    // 항상 직전 랜덤 수는 전역변수로 저장하고 매번 비교한다
