...
/**************************************************************************************************************************
    [ 패럴렉스 기능 구현하기 ]
    1. 정의 : 패럴렉스란 스크롤 작동시 같은 방향으로 요소가 다른 속도를 가지고 움직이므로 사용자가 공간감을 느낄 수 있게 하는 구현방법
    2. 방법 : 
        (1) 범위 - 요소가 화면에 등장하여 보일 동안 스크롤 될 때 이동함
        (2) 움직일 크기 설정이 필요하다
...
function moveEl(elPos, ele, setH){
    ...
    if(elPos < winH && elPos > -200){
        // 1. 위치이동값 계산
        // 실제이동값 = 정한범위 - (위치값 * 정한 범위 / 전체범위)
        let x = setH - (elPos * setH / winH);
        console.log('x:',-x);

        // 2. 해당 요소의 위치값 이동 CSS 반영하기
        // Y축 이동시 윗쪽방향은 마이너스임
    ...
    /*************************************************************************************** 
        ...
        (비례식으로 실제 이동값 알아내기)
        전체범위 : 위치값 = 정한범위 : 실제이동값
        실제이동값 = 위치값 * 정한 범위 / 전체범위

        -> 그런데, Y축 위치이동은 처음에 0부터 서서히 커지므로 이동수치값은 정한범위에서 실제이동값을 빼야함
        실제이동값 = 정한범위 - (위치값 * 정한 범위 / 전체범위)
