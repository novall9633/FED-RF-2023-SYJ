...
// 2.이벤트 설정 : 이벤트 종류는 mouseenter(경계선안에 들어옴)
evtBox.forEach((ele,idx)=>{
    dFn.addEvt(ele,'mouseenter',()=>seeMe(idx));
});

// 3.함수만들기
function seeMe(seq){//순번받기
...
dFn.addEvt(eBox,'mouseenter',function(){
    ...
    'url(./images/1601098897635.gif) no-repeat 0/100% 100%';
...
/* 
    ...
    -> 둘의 차이는 이벤트 버블링에 있다
    -> 경계선 기준의 이벤트인 mouseenter / mouseleave는 자체요소에서만 발생하고 버블링되지 않는다
    -> 자손요소에서 버블링되어 발생하는 mouseover/ mouseout으로 셋팅할 경우 빈번한 이벤트 발생이 문제가 될 경우
