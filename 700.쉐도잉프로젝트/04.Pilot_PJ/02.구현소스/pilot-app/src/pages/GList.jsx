...
export function GList() {
    // 참조 변수 셋팅 : 리랜더링없이 값 유지
    // 1. 아이템 코드 (m1,m2,m3,...)
    const item = useRef("m1");
    // 2. 카테고리명(man/women/style)
    const catName = useRef("men");
    // 리랜더링을 위한 상태변수 : 무조건 리랜더링설정
        ...
        gdata.map((v, i) => (
            <div key={i}>
                ...
                >
                    ...
                    <aside>
                        <h2>{v.ginfo[1]}</h2> <h3>{addComma(v.ginfo[3])}원</h3>
                    </aside>
                </a>
            </div>
        ));
    ...
    return (
        <>
            <main id="cont">
                <section>
                    <div className="tit">ALL ITEMS LIST</div>
                    <div id="optbx">
                        <label htmlFor="men">남성</label> <input type="checkbox" id="men" defaultChecked />
                        <label htmlFor="women">여성</label>
                        <input type="checkbox" id="women" defaultChecked /> <label htmlFor="style">스타일</label>
                ...
                >
                    <ItemDetail cat={catName.current} goods={item.current} />
                </div>
            </main>
        </>
    );
