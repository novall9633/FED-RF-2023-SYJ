...
export const CartList = memo(({ selData, flag }) => {
    ...
    const deleteItem = (e) => {
        ...
        let confMsg = "해당 제품을 장바구니에서 지우시겠습니까?";
        // 지울지 여부를 사용자에게 물어본다
    ...
    const goResult = (e) => {
        ...
        let cidx = $(e.currentTarget).attr("data-idx");
        ...
        // 해당 데이터 업데이트하기
        // forEach로 돌리면 중간에 맞을 경우 return 할 수 없음
        // 일반 for문으로 return 또는 continue사용 가능

        // ->>>>some()이라는 메서드가 있다.
        // return true로 조건에 처리시 for문을 빠져나옴(return과 유사)
        // return false로 조건 처리시 for문을 해당순번 제외하고 계속 순회함(continue와 유사)
        // 참고 : https://www.w3schools.com/jsref/jsref_some.asp

        // Array some() 메서드 테스트
        // cartData.some((v) => {
        //   console.log("some test 상단:",v.idx);
        //   // if(v.idx==17){return true;} // -> for문 break 유사
        //   if(v.idx==17){return false;} // -> for문 continue 유사
        //   console.log("some test 하단:",v.idx);
        // });

        // 클릭 시 'data-idx'값에 업데이트 할 요소 idx번호 있음 -> cidx
        cartData.some((v, i) => {
            // 해당 순번 업데이트 하기
            if (v.idx == cidx) {
                // 업데이트 하기
