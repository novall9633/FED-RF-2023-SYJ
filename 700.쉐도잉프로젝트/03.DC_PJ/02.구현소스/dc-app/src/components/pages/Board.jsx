...
export function Board() {
    ...
    *************************************/
    const chgMode = (e) => {
        ...
        //   3-7. 삭제하기
            ...
            if (window.confirm("정말로 글을 삭제하시겠습니까?")) {
                // 1.데이터 순회하다가 해당데이터이면 순번으로 splice(순번,1) 사용 삭제
                orgData.some((v, i) => {
                    if (Number(cData.current.idx) === Number(v.idx)) {
                        // 해당 데이터의 순번으로 삭제
                        orgData.splice(i, 1);
    ...
    const compUsr = (usr) => {
        ...
        if (myCon.logSts !== null) {
            ...
            // 아이디로 조회함!
            if (cUser) {
                // 할당 안되면 undefined이므로 할당되었을때만 if문처리
    ...
    /************************************* 
      ...
      조건 : 
        ...
        (이때 배열은 세션스에 기록함! 이유는 브라우저 닫을 때 사라짐)
    ...
    *************************************/
    const plusCnt = () => {
        //0. 처음에 통과상태 설정하기
        let isOK = true;
        // 세션스에 등록된 글 or 로그인사용자 글 일때 false처리
    ...
    // 사라질 때 불러준다
    useEffect(() => {
        // 처음 한번 들어왔을때 내림차순 정렬gyrhk dlTsms rk?
