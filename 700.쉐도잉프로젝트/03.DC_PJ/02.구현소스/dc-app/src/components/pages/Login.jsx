...
export function Login() {
    ...
    // 3. 전체 유효성 검사 함수 /////////////////
    const totalValid = () => {
        // 3-1. 모든 상태변수 빈값일 때 에러상태값 업데이트(상태변수 검사)-> 입력 전 상태는 false
        // -> 빈값일 경우 에러처리함
        if (!userId) setUserIdError(true);
        if (!pwd) setPwdError(true);
        // 3-2. 통과시 true, 불통과시 false 리턴처리
        // 통과조건 : 빈값이 아님 + 에러후크변수가 모두 false
    ...
    const onSubmit = (e) => {
        ...
        if (totalValid()) {
            console.log("성공");
            // DB 역할의 로컬스에 데이터를 비교한다

            // 만약 로컬스에 'mem-data'가 없으면
            // 초기화-> 함수 내에 이미 걸러내고 있음
            initData();
            ...
            // 결과 리턴이 없는 경우 undefined이므로 false
            if (findD) {
                // 같은 아이디가 있는 경우/////
                console.log("아이디같아요");
                // 아이디 에러 상태 업데이트
                ...
                if (findD["pwd"] === pwd) {
                    ...
                    myCon.setLogMsg("Welcome "+findD.unm+usrIcon[Math.floor(Math.random()*usrIcon.length)]); 
                    ...
                    <ul>
                        ...
                        <li>
                            ...
                            />
                            {
                                ...
                                pwdError && (
                                    <div className="msg">
                                        <small style={{ color: "red", fontSize: "10px" }}>{msgPwd}</small>
                                    </div>
                                )
                            }
                        </li>
