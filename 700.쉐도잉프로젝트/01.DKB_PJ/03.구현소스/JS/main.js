...
function showMv() {
    ...
    console.log("showMv");
    // 동영상 넣기
    // 대상 : 나 자신(.intro-mv-img)
    this.innerHTML = `
        <video src='./images/intro-mv.mp4' controls autoplay></video>
    `;
...
let preNewData = previewData.sort((x, y) => {
    ...
    // 배열값 중 idx 속성값을 가져와서 숫자형변환 사용()
    let a = Number(x.idx);
    let b = Number(y.idx);

    /// 배열 순서 변경 메서드인 sort()내부에 return값을 사용하여 순서를 변경한 새로운 배열을 만들어준다
    return a == b ? 0 : a > b ? -1 : 1;
    //
});
console.log(preNewData);
// 2. 화면 대상에 태그 만들어 넣기
// 대상 선정 : .preview-box>div
...
///////////////최신동영상 파트 이동기능 구현/////////////////////
// 1. 요구사항 : 버튼 한번에 한 영상씩 이동 양쪽끝에 가면 이동중단 해당 방향 버튼 사라짐

// 2. 대상선정
// 2-1. 이벤트 대상 : .btn-box button
const btnClip = dFn.qsa(".btn-box button");
// 2-2. 변경 대상 : .clip-box ul
...
// 3-4. 이동 단위수 : 간격 이동까지 고려한 한번에 이동할 단위 -25.5%
...
// 이때 배열값을 문자열값으로 변환하는 join()을 사용하여 연결자를 빈값으로 처리하면 배열의 구분자 콤마가 없는 태그로만 연결된 순수한 태그 결과 문자열이 만들어진다.
